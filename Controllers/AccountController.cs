...
File Contents:
    This class is a controller to handle user authentication
 */
...
namespace LMS.Controllers
{
    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            switch(role)
            {
                case "Administrator": return AddAdministrator(fName, lName, DOB);
                    break;
                case "Professor": return AddProfessor(fName, lName, DOB, SubjectAbbrev);
                    break;
                case "Student": return AddStudent(fName, lName, DOB, SubjectAbbrev);
                    break;
                default:
                    return "";
            }
        ...
        private static string toStringUID(int UID)
        {
            string output = "u" + UID.ToString().PadLeft(7, '0');
            return output; 
        }

        private string AddAdministrator(string fName, string lName, DateTime DOB)
        {
            Administrators admin = new Administrators();
            admin.FName = fName;
            admin.LName = lName;
            admin.Dob = DOB;
            int UId = (from administrator in db.Administrators select administrator.UId).Count() +
                      (from professor in db.Professors select professor.UId).Count() +
                      (from stud in db.Students select stud.UId).Count();
            
            admin.UId = toStringUID(UId);
            db.Administrators.Add(admin);
            db.SaveChanges();
            return admin.UId;
        }

        private string AddProfessor(string fName, string lName, DateTime DOB, string SubjectAbbrev)
        {
            Professors prof = new Professors();
            prof.FName = fName;
            prof.LName = lName;
            prof.Dob = DOB;
            int UId = (from professor in db.Professors select professor.UId).Count() +
                      (from administrator in db.Administrators select administrator.UId).Count() +
                      (from stud in db.Students select stud.UId).Count();

            prof.UId = toStringUID(UId);
        ...
        private string AddStudent(string fName, string lName, DateTime DOB, string SubjectAbbrev)
        {
            Students student = new Students();
            student.FName = fName;
            student.LName = lName;
            student.Dob = DOB;
            int UId = (from stud in db.Students select stud.UId).Count() +
                      (from professor in db.Professors select professor.UId).Count() +
                      (from administrator in db.Administrators select administrator.UId).Count();

            student.UId = toStringUID(UId);
